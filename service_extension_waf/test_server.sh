...
pytest test_server.py::test_server -sv \
    --se_test_case="Verify ipv4 addresses are converted to cidr range allowed ranges are allowed" \
    --se_result="pass" \
    --se_headers='[{":host":"se-waf.demo.com"},{"x-forwarded-for":"1.1.1.1,2.2.2.2"}]' \
    --se_allowed_ipv4_cidr_ranges='1.1.1.1'
...
pytest test_server.py::test_server -sv \
    --se_test_case="Verify more specific CIDR ranges are denied" \
    --se_result="fail" \
    --se_headers='[{":host":"se-waf.demo.com"},{"x-forwarded-for":"1.1.1.1,2.2.2.2"}]' \
    --se_denied_ipv4_cidr_ranges='1.0.0.0/8' \
